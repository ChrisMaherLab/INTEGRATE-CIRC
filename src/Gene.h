...
public:
	...
	//New
	//Get the exon that explains the fusion.
	//Attempt to get an exon from a previously identified transcript if it is a match,
	//else return to the shortest exon
	int getBestExon3(int git, int pos, int isbkLeft, int& is5p, int& tid, int & strand,
		int & pos1, int & pos2, string & name, int & exonNum, 
		string gene5pName, string currGeneName, int tranId5p, int tranId3p, int & bestTranId);

	bool posIsValid(int git, int pos);
};
