...
bool Gene::posIsValid(int gid, int pos) {
	int min = 276447232;
	int max = -1;
	for(int i=0;i<genes[gid].transIds.size();i++) {
		int tranId = genes[gid].transIds[i];
		int count = transcripts[tranId].exonCount;
		for(int j=0;j<count;j++) {
			int s = transcripts[tranId].exonStarts[j];
			int e = transcripts[tranId].exonEnds[j];
			if(s < min)
				min = s;
			if(e < min)
				min = e;
	...
	int lowerbound = min - 20000;
	int upperbound = max + 20000;
...
int Gene::getBestExon3(int gid, int pos, int isbkLeft, int& is5p, int& tid, int & strand,
	int & pos1, int & pos2, string & name, int & exonNum,
	string gene5pName, string currGeneName, int tranId5p, int tranId3p, int & bestTranId) {
...
//					cout<<"change"<<endl;
	...
	//Attempt to ensure that the exon number that was selected came from the same
	//transcript that the main junction exon number came from, so that they
	//are comparable - even if that transcript does not have the shortest exon
	int targetTranId = tranId3p;
	if(gene5pName == currGeneName) 
		targetTranId = tranId5p;
	if(bestTranId != targetTranId) {
		int count = transcripts[targetTranId].exonCount;
		for(int i=0;i<count;i++) {
			int pp1 = transcripts[targetTranId].exonStarts[i];
			int pp2 = transcripts[targetTranId].exonEnds[i];
			if(isbkLeft==1) {
				if(abs(pos-pp1)<=best) {
					name = transcripts[targetTranId].name;
					pos1=pp1;
					pos2=pp2;
					if(strand==0) {
						exonNum=i+1;
					...
					bestTranId = targetTranId;
					cout<<"Reset using targetTran"<<endl;
				}
			} else {
				if(abs(pos-pp1)<=best) {
					name = transcripts[targetTranId].name;
...
int Gene::getBestExon(int gid, int pos, int isbkLeft, int& is5p, int& tid, int & strand,
		int & pos1, int  & pos2, string & name, int & exonNum, int & bestTranId) {
//cout<<"$$$$$$$$$$$$$$$$$$$$$$$$$"<<endl;
