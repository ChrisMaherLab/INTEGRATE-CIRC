...
int BreakPoint::getBreakPoints(vector<break_point_record_t> & bkvec, char * file,char * file1,char * file2, char * refname, Reference & ref, char * sample_name, char* dir){
    ...
    outFile2<<"##INFO=<ID=IMPRECISE(Fusion Junction),Number=0,Type=Flag,Description=\"Imprecise structural variation or other junction\">\n";
...
// bnd
    ...
    if(imprecise==1)
    {
        vt.info=vt.info+";IMPRECISE(Fusion Junction)";
        vt2.info=vt2.info+";IMPRECISE(Fusion Junction)";
    }
    vt.format="GT";
    vt2.format="GT";
