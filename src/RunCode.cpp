...
void usageBuild()
{
	...
	cout<<endl;
	cout<<"    Integrate-circ mkbwt (options) reference.fasta"<<endl;
	cout<<endl;
	cout<<"    options:"<<endl;
        cout<<endl;
...
void usageFusion()
{
    ...
    cout<<"         -bedpe    string  : File to store all fusions in SMC-RNA bedpe format                          default: junctions.bedpe"<<endl;   
	...
	cout<<endl;
	cout<<"Integrate-circ will only use sequences in reference.fasta. "<<endl;
	...
	cout<<endl;
	cout<<"For dna bams: If solt-clips are provided, then Integrate-circ is trying to search rearrangement breakpoints, otherwise, only paired reads may be included in the analysis."<<endl;
	cout<<endl;
	...
	cout<<"e.g. Integrate-circ fusion -normal (options) reference.fasta annotation.txt directory_to_bwt accepted_hits.normal.bam unmapped.normal.bam (dna.normal.bam)"<<endl;
	exit(0);
}
...
*/
		...
		if(tmp.compare("-fcirc")==0)
			{
				if(i+1<argc)
                        {

                                
                                opt.fcircFile=argv[i+1];
                        ...
                        {
                                cout<<"Please give a file name after -fcirc"<<endl;
                                exit(1);
                        }
			}
...
int RunCode::runFindFusions(int argc, char * argv[]) {
		...
		if(opt.isRunningNormal==1) //Should never be true. Deprecated
		{
		...
		else
		{
			...
			opt.bkFileBEDPE="bk_fusion.bedpe";//deprecated
			opt.bkFileVCF="bk_sv.vcf";
            opt.fileJunction="junctions.bedpe";//deprecated
		...
		t=clock();
		cout<<"Estimating how repetitive the remaining reads are..."<<endl;
		...
		result.checkALLPrime(g); //Also assigns reciprocal / fcircRNA status
