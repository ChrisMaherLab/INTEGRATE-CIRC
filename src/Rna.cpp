...
int printType(int tp)
{
    ...
    {
        case 0:
            cout<<"Inter_Chromosomal_Fusion";
            break;
        case 1:
            cout<<"Intra_Chromosomal_Fusion";
            break;
        case 2:
            cout<<"Read_Through";
            break;
        case 3:
            cout<<"Splice_Variant_Junction";
            break;
        case 4:
            cout<<"Backsplice_Junction";
            break;
        case 5:
            cout<<"Unknown_Junction";
            break;
       // case 3:
...
int Rna::clusterAndRemove(Gene & g, vector<int> const& spIds,  vector<int> const& enIds,  int cutoff, int gid1, int gid2, int bacc)
{
    ...
    //tmp is a vector of all sprna on the edge
    sort(tmp.begin(),tmp.end(),mySortSplit);

    vector<int> ids; //Will include all sprnas that will be kept
    int id=0;
...
//cout<<"size="<<tmp.size()<<endl;
    for(int j=0;j<=tmp.size();j++) //For each sprna
    {
//cout<<"j="<<j<<endl;
        number++; //count how many sprnas have been looked at

        if(j>0 && j<tmp.size()) //If this isn't the first sprna
        {
        pp1=tmp[j].pos1; 
        if(tmp[j].bkLeft1==0)
            pp1+=tmp[j].len1-1; 
        ...
        if(tmp[j].bkLeft2==0)
            pp2+=tmp[j].len2-1;
        //pp1 and pp2 are positions of end of sprnas
        ...
        //ppm1 and ppm2 are the positions of the previous sprna in the tmp vector of sprnas
//cout<<"here"<<endl;
        //if not the first or last item in the list &&
        // the positions of the new sprna is further than 5 from the previous sprna
        //This defines a single cluster
        if((j>0 && j<tmp.size() && !(( pp1<=ppm1+5 && pp1>=ppm1-5 && pp2<=ppm2+5 && pp2>=ppm2-5) || ( pp2<=ppm1+5 && pp2>=ppm1-5 && pp1<=ppm2+5 && pp1>=ppm2-5))) || j==tmp.size() )
        {
            int num=number-1; //num = number of sprnas in the cluster

//cout<<"in here"<<endl;
            int localCutOff=cutoff; //cutoff refers to the distance below which a fusion will be called a read-through
            int hasMaxSmall=0;
            
	    int isBoth=0;
	    if(isCanonical(g, tmp[j-1], bacc)==1) //bacc refers to how close something needs to be to the annotation to be considered canonical
            {
		isBoth=1;
                hasMaxSmall=1;
                localCutOff=1;
            }
            if(num>=localCutOff) //Unclear
            {
...
struct TraverseCluster {
    ...
    bool operator()(int x1, int x2, FusionEdge& edge) {
        if (!edge.spannings.empty()) {
            //cout<<"edge.spannings is not empty, goint to clusterAndRemove"<<endl;
...
struct TraversePrint {
    ...
    bool operator()(int x1, int x2, FusionEdge const& edge) {
        if(!edge.spannings.empty())
        {
            ...
            cout<<"Adding "<<rt.nm5p<<">>"<<rt.nm3p<<" to results"<<endl;
            objRst.addResult(rt);
        } 
...
int Rna::traversePrint(Gene & g, Reference & ref, Result & result, int minIntra, int bacc) {
    //tp is function object with overloaded operator()
